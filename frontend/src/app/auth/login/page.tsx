...
export default function Login() {
  ...
  const InputStyle =
    'w-full px-4 py-2 border border-green-300 bg-green-50 text-green-900 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500';
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        if (err.response?.status === 403) {
          ...
          setError(
            'Something went wrong during login, check credentials. Please try again.'
          );
        }
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-green-50 px-4">
      <div className="w-full max-w-md bg-white shadow-lg rounded-xl p-8 space-y-6">
        ...
        <Link
          href="/auth/register"
          className="text-green-600 hover:underline pl-3 mr-10"
        >
          Or Register{' '}
        </Link>
      </div>
