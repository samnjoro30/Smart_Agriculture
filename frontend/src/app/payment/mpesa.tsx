...
const MpesaPayment: React.FC = () => {
  ...
  const [paymentStatus, setPaymentStatus] = useState<
    'idle' | 'pushing' | 'awaiting-pin' | 'success' | 'error'
  >('idle');
  ...
  const checkPaymentStatus = async (checkoutRequestID: string) => {
    ...
    const interval = setInterval(async () => {
      ...
      if (attempts >= maxAttempts) {
        ...
        setIsLoading(false);
        alert(
          'Payment timeout. If you paid, your account will update shortly.'
        );
      }
    ...
    <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 h-full flex flex-col justify-between">
      ...
      <div className="space-y-3">
        <button
          ...
          disabled={
            phoneNumber.length < 9 || isLoading || paymentStatus === 'success'
          }
        ...
        >
          ...
          {paymentStatus === 'awaiting-pin' && (
            <span className="text-[10px] font-medium animate-pulse">
              Enter PIN on your Safaricom line
            </span>
          )}
        </button>
