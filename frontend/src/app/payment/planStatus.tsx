...
import { ShieldCheck, Zap, Activity, ChevronRight, Loader2 } from 'lucide-react';
...
const UserPlanStatus: React.FC<UserPlanStatusProps> = ({
  ...
  const getStatusTheme = () => {
    if (usagePercentage >= 90) return { color: 'text-red-600', bg: 'bg-red-500', lightBg: 'bg-red-50' };
    if (usagePercentage >= 75) return { color: 'text-amber-600', bg: 'bg-amber-500', lightBg: 'bg-amber-50' };
    return { color: 'text-green-600', bg: 'bg-green-500', lightBg: 'bg-green-50' };
  };
      ...
      <div>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-2xl font-black text-gray-900 tracking-tight">{currentPlan}</h3>
            {currentPlan !== 'Basic' && <ShieldCheck size={20} className="text-blue-500" />}
          </div>
          <p className="text-sm font-medium text-gray-500">
            {currentPlan === 'Basic' ? 'Free tier for small setups' : 'Premium farm management'}
          </p>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-end">
            <span className="text-xs font-bold text-gray-700 uppercase tracking-tight">Herd Capacity</span>
            <span className="text-sm font-black text-gray-900">
              {animalCount} <span className="text-gray-400 font-medium">/ {maxAnimals}</span>
            </span>
          </div>
        ...
        <button className="w-full group bg-gray-50 hover:bg-green-600 hover:text-white border border-gray-100 rounded-2xl p-4 transition-all flex items-center justify-between active:scale-[0.98]">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-xs font-black uppercase tracking-tight">Upgrade Plan</p>
