...
async def register_farm(payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    existing_user = await get_user_by_email(payload.username, db)
    if existing_user:
    ...
    return {"message": f"User registered {payload.username} successfully"}
...
async def Verify_farmer(request: Request, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if user.otp_expires_at and datetime.utcnow() > user.otp_expires_at:
            raise HTTPException(
                status_code=401, detail="Otp has expired, try resend new otp"
            )
...
async def RegisterForNewsLetter(
    request: RegisterSubscribers, db: AsyncSession = Depends(get_db)
):
    user_news = {
        "email": request.email,
    }
...
async def resendCode(request: codeResend, db: AsyncSession = Depends(get_db)):
    body = await request.json()
    email = body.get("email")
    ...
    return {"message": "Code resend successfull"}
