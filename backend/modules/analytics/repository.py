...
async def get_monthly_analytics(db: AsyncSession, user_id: UUID, year: int, month: int):
    ...
    cow_stmt = (
        select(MilkRecord.cow_id, func.sum(MilkRecord.liters).label("yield"))
        .where(
            MilkRecord.user_id == user_id,
        ...
        .group_by(MilkRecord.cow_id)
        .order_by(text("yield DESC"))
        .limit(5)
    )
...
async def get_session_performance(
    ...
    query = (
        select(MilkRecord.session, func.sum(MilkRecord.total_revenue).label("revenue"))
        .where(
            MilkRecord.user_id == user_id,
            extract("year", MilkRecord.created_at) == year,
            extract("month", MilkRecord.created_at) == month,
...
async def get_top_performing_cows(
    db: AsyncSession, user_id: UUID, year: int, month: int
):
    query = (
        select(
