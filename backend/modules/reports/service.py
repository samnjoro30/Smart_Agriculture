...
class ReportService:
    ...
    async def get_financial_ledger_data(
        ...
        return FinancialReportResponse(
            ...
            farm_name="",  # You can fetch this from the User object
    ...
    async def get_report_by_id(db: AsyncSession, report_id: UUID, user_id: UUID):
        report = await ReportRepository.get_report_by_id(db, report_id, user_id)
    ...
    async def generate_receipt_pdf(db: AsyncSession, payment_id: UUID, user_id: UUID):
        ...
        return {
            "message": "PDF generation logic goes here",
            "receipt_no": payment.mpesa_receipt_number,
        }

    @staticmethod
    async def get_report_pdf_binary(db: AsyncSession, report_id: UUID, user_id: UUID):
        ...
        template_context["farm_name"] = user_id.farmname
        template_context["start_date"] = start_date
        template_context["end_date"] = end_date

        template_name = (
            "financial_report.html"
            if report_record.report_type == "FINANCIAL_SUMMARY"
            else "payment_receipt.html"
        )

        # 5. Use the Utility to generate bytes
        pdf_bytes = PDFUtility.render_to_pdf(template_name, template_context)
