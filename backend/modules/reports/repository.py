...
from modules.payments.model import PaymentTransaction, PaymentCheck # Import from your other modules

class ReportRepository:

    @staticmethod
    async def create_report_record(db: AsyncSession, user_id: UUID, report_type: str, parameters: dict):
    ...
    async def get_report_by_id(db: AsyncSession, report_id: UUID, user_id: UUID):
        ...
        stmt = select(ReportRecord).where(ReportRecord.id == report_id, ReportRecord.user_id == user_id)
    ...
    @staticmethod
    async def update_report_status(db: AsyncSession, report_id: UUID, status: str, file_url: str = None):
