...
async def check_payment_status(
    ...
    try:
        response = await get_payment_by_checkout_id(
            db, checkout_request_id, current_user
        )

        return {"message": "Payment status fetched", "data": response}
...
async def stk_push_callback(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    logger.info(
        "STK Push callback processed",
        status_code=200,
        checkout_request_id=callback_response.get("checkout_request_id"),
    )
