...
def downgrade() -> None:
    ...
    op.create_index(op.f('idx_refresh_token_token'), 'refresh_token', ['token'], unique=False)
    op.create_index(op.f('idx_refresh_token_lookup'), 'refresh_token', ['token'], unique=False)
    op.alter_column('livestock', 'tag',
               existing_type=sa.VARCHAR(length=150),
               nullable=False)
